...
def fetchBianceData(ticker :str, timeInterval :str, startDate :str, endDate :str, stableCoin='USDT'):
    ...
    try:
        ...
        df.to_csv(f'data/{ticker}{stableCoin}_{timeInterval}_raw.csv', index=False)

        print(f'Data fetched and succesfully saved at data/{ticker}{stableCoin}_{timeInterval}_raw.csv')
        
        return pd.read_csv(f'data/{ticker}{stableCoin}_{timeInterval}_raw.csv')

    except Exception as e:
        return(f'Data fetch failed {e}')


def generateFeatures(ticker :str, timeInterval :str, predictionLength :int, historicalLength :int, stableCoin='USDT'):
    df = pd.read_csv(
        f'data/{ticker}{stableCoin}_{timeInterval}_raw.csv',
    ...
    df.to_csv(f'data/{ticker}{stableCoin}_{timeInterval}_features.csv', index=False)
    
    # to reset index for slicing in labeling, send new DF to avoid referencing
    df = pd.read_csv(f'data/{ticker}{stableCoin}_{timeInterval}_features.csv')
...
def generateTrainTestDatasetBinary(df :pd.DataFrame, ticker :str, timeInterval :str, stableCoin='USDT', testSize=0.3):
    ...
    print('Binary xTrain, yTrain, xTest, yTest saved in data and returned')

    return(xTrain, yTrain, xTest, yTest)


def generateTrainTestDatasetMultiLabel(df :pd.DataFrame, ticker :str, timeInterval :str, stableCoin='USDT', testSize=0.3):
    numberOfClasses=3
    train, test = train_test_split(df, test_size=testSize)
    ...
    print('Multi label xTrain, yTrain, xTest, yTest saved in data and returned')
